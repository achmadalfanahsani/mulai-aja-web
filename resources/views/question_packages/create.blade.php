                ...
                <form action="{{ route('question-packages.store') }}" method="POST" class="py-3">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  ...
                                  placeholder="Tuliskan petunjuk pengerjaan atau rincian cakupan materi ujian...">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select class="form-select @error('package_type') is-invalid @enderror" id="package_type" name="package_type" required>
                            ...
                            @endphp
                            <option value="multiple_choice" {{ old('package_type', $requestedType) == 'multiple_choice' ? 'selected' : '' }}>Pilihan Ganda</option>
                            <option value="essay" {{ old('package_type', $requestedType) == 'essay' ? 'selected' : '' }}>Isian Singkat (Essay)</option>
                            <option value="mixed" {{ old('package_type', $requestedType) == 'mixed' ? 'selected' : '' }}>Campuran</option>
                        </select>
                        <small class="text-muted">Pilih tipe soal yang akan ada di dalam paket ini agar terorganisir dengan baik.</small>
                        @error('package_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="block block-rounded block-bordered bg-body-light mb-4 p-3">
                        ...
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" id="shuffle_questions" name="shuffle_questions" value="1" checked>
                        ...
                        <div class="form-check form-switch mt-3">
                            <input class="form-check-input" type="checkbox" id="shuffle_answers" name="shuffle_answers" value="1" checked>
